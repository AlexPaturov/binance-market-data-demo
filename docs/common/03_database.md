...
- **Пачками, а не всю очередь сразу:** проход обязан укладываться в командный таймаут (600 с). Иначе транзакция откатится целиком, и работа не сдвинется вовсе — ровно это случилось 13.07.2026 со старой процедурой.
- **Возвращает:** количество пересчитанных свечей.
- **Взаимодействие:** [Читает] → `"Trades"`; [Читает + Пишет] → `"DirtyMinutes"`, `"Ohlcv_1min"`, `"Processing_Watermarks"`.

> Почему очередь, а не статус тиков — см. [ADR 0009](../adr/0009-dirty-minutes-queue.md). Почему не watermark — [ADR 0004](../adr/0004-watermarking-idempotency.md): watermark как фильтр корректности означал, что докачанная в дыру сделка **никогда** не попадёт в свечу.
...
**Механика.** Два разных механизма на двух звеньях:

| Процесс | Где берёт работу | Пишет | Куда ставит работу дальше |
| :--- | :--- | :--- | :--- |
| `sp_aggregate_dirty_minutes` | очередь `"DirtyMinutes"` | `"Ohlcv_1min"` | `ProcessingStatus = 'new'` у пересчитанных свечей |
| `FeatureCalculatorWorker` + `sp_upsert_ohlcv_features` | свечи `"Ohlcv_1min"` со статусом `'new'` | `"Ohlcv_Features"` | — |
