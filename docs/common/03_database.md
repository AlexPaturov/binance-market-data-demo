...
- **Логика:**
  1. Забирает из `"DirtyMinutes"` до `p_max_minutes` минут, от старых к свежим, под `FOR UPDATE SKIP LOCKED` — параллельный запуск возьмёт следующие, а не будет ждать.
  2. **Пересчитывает свечу целиком** из всех тиков минуты, включая те, что уже участвовали в прошлом расчёте. Порядок прихода данных не имеет значения, операция идемпотентна.
...
| `IX_DirtyMinutes_OpenTime` | `DirtyMinutes` | `(OpenTime)` | Разбор очереди от старых минут к свежим. |
