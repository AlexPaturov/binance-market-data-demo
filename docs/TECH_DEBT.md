...
## 3. Дублирование SQL проверок качества
...
## 4. `ConfigureKestrel` конфликтует с `ASPNETCORE_URLS`
...
## 5. Hangfire Dashboard в DataManager
...
## 6. Отмена импорта логируется как ошибка
...
## 7. Второй mount `bdc_data` в `bdc_worker`
...
## 8. Проверка границы ретенции в воркерах
