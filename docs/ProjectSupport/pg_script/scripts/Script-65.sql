...
SELECT 
    ROW_NUMBER() OVER (ORDER BY "MissingTrades_Count" DESC) AS "Gap_Number",
    ...
    -- Добавляем общий счетчик пропусков
    COUNT(*) OVER() AS "Total_Gaps_Found"
FROM GapsFound
ORDER BY "MissingTrades_Count" DESC;
