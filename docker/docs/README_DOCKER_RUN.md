...
Скрипты поднимают инфраструктуру (Postgres, PgBouncer, RabbitMQ, Seq) и создают директории под архивы и CSV.
...
> Запускать из корня проекта. Данные Postgres — в docker volume `dev_postgres_data`; схема накатывается автоматически из `docker/postgres/init/` при первом старте.
