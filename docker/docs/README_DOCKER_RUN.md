...
## ⚡ 1.1 Быстрый запуск через скрипты (рекомендуется для локальной разработки)

Скрипты автоматически монтируют/размонтируют внешний диск с данными Postgres (`/mnt/ext`) и паркуют головки HDD при остановке.
...
Остановка инфраструктуры + размонтирование диска:
```bash
./docker/dev-stop.sh
```

> Запускать из корня проекта. Диск `/dev/sda1` монтируется автоматически если ещё не примонтирован.

---

## 🧪 1.2 Запуск ТОЛЬКО инфраструктуры для локального запуска DataManager и Worker из IDE
